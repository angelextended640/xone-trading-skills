...
> 🇻🇳 **Primary market: Vietnam (HOSE / HNX / UPCOM).** For the Vietnamese guide — the **primary recommended starting path** — see **[`README.vi.md`](README.vi.md)**. This English README covers the international advanced-user track and contributor reference. New users on the VN market should start with `README.vi.md`.

Xone Trading Skills started as a personal project to use AI to improve trading processes. As of 2026-05, the project has been **repositioned with Vietnam as the primary market**, with a family of `vn-*` skills built on the open-source `vnstock` data layer. The international US skills documented below are **retained for advanced multi-market users**. Japanese localisation was fully removed on 2026-05-13.

## At a Glance

| Stat | Value |
| --- | --- |
| VN skills shipped | **21** (full list below; latest additions: `vn-canslim-screener`, `vn-earnings-analyzer`, `vn-pead-screener`, `vn-margin-rules-monitor`, `vn-etf-screener`, `vn-daily-brief` — Phase B + Phase D stretch) |
| VN tests passing | **335+/335+** |
| International skills | ~54 (US-focused; FMP / FINVIZ / Alpaca-backed where applicable) |
| Primary data layer (VN) | [`vnstock`](https://github.com/thinh-vu/vnstock) — open source, free, no API key |
| Primary data layer (US) | FMP API, FINVIZ (optional), Alpaca (optional) |
| Languages | English (international), Vietnamese (VN skills' UI / reports) |
| License | MIT |

Xone Trading Skills is a Claude Skills-based trading workflow toolkit for time-constrained individual investors.
...
📖 **Documentation site:** <https://xonevn-ai.github.io/xone-trading-skills/>

**Project vision:** [`PROJECT_VISION.md`](PROJECT_VISION.md)

**Vietnamese README (primary):** [`README.vi.md`](README.vi.md)
...
### Vietnam market (primary — `vn-*` family)

| Skill | What it does | Tests |
| --- | --- | ---: |
| **`vn-market-mechanics`** | Canonical reference for VN: ±7%/±10%/±15% price bands, T+2.5 settlement, lot size 100, broker fees, 0.1% sale tax, foreign room. Every other `vn-*` skill cites this. | knowledge-only |
| **`vn-position-sizer`** | Sizes long trades with VN rules — lot 100 rounding, ceiling/floor warning, fee+tax accounting. Fixed Fractional / ATR-based / Kelly. | 35 |
| **`vn-data-fetcher`** | CLI wrapper for `vnstock`. Subcommands: `ohlcv`, `info`, `dividends`, `fundamentals` (quarter/year), `foreign-flow` snapshot. CSV cache under `state/vn_market_data/`. `--fixture` mode for offline tests. | 33 |
| **`vn-portfolio-manager`** | Manual-entry VND portfolio. After-fee/after-tax P&L, sector breakdown, 10%/30% concentration warnings. Pipes prices from `vn-data-fetcher`. | 25 |
| **`vn-foreign-room-tracker`** | Tracks foreign ownership room (a VN-only concept) across a watchlist. Alerts: `full`, `released`, `spike_up`, `spike_down`. Append-only history. | 27 |
| **`vn-sector-analyst`** | VN-Index sector rotation. 11 sectors × ~80 most-traded stocks. Return 5D/20D/60D, RS vs VN-Index, regime hint (Banking + Real Estate concordance). | 23 |
| **`vn-breakout-trade-planner`** | Integrative long-trade planner. Pivot/stop/R-targets, T+2.5 sellable-day calendar (business-day aware), optional sector + foreign-room context. | 28 |
| **`vn-tax-fee-calculator`** | Itemised broker fees (10 brokerages) + 0.1% sale tax + 5% dividend tax + custody + advance-cash fee. `compare` subcommand benchmarks net P&L across brokers. | 25 |
| **`vn-news-analyst`** | WebSearch-based VN news brief. Tier 1 sources (CafeF, Vietstock, NDH). Categorises into monetary policy / regulation / sector / company / macro; maps to affected tickers. | knowledge + workflow |
| **`vn-vcp-screener`** | Minervini-style VCP (Volatility Contraction Pattern) screener adapted for HOSE ±7% band. Detects successive contractions with declining depth + volume dry-up. Grade A/B/C. | 18 |
| **`vn-pullback-screener`** | Lower-risk swing entry alternative to VCP. Identifies uptrending stocks pulling back to MA20 / MA50 with RSI 35-50 and volume dry-up. | 18 |
| **`vn-economic-calendar`** | Reference for SBV / GSO release schedules, FOMC dates that move USDVND, ex-rights dates, AGM season, Tết / quarter-end patterns. Drives event-aware sizing. | knowledge-only |
| **`vn-dividend-screener`** | Sustainable dividend-stock screener for Core (income) portfolio. Yield + payout sustainability + 3y dividend growth + ROE / D/E quality + EPS trajectory. Yield-trap detection (high yield + collapsing EPS auto-reject). | 26 |
| **`vn30-derivatives-planner`** | VN30 Index Futures planner — `roll` (3rd-Thursday calendar + basis), `hedge` (beta-adjusted contract count), `plan` (short/long full trade plan with IM check), `cost` (10 broker comparison). T+0 settlement, no sale tax. **The only legal short-exposure tool on VN markets.** | 39 |
| **`vn-trader-memory`** | Plan → Trade → Record → Review thesis lifecycle. Register from any `vn-*screener`, IDEA→ENTRY_READY→ACTIVE→CLOSED, attach `vn-position-sizer` output, close with VND gross+fee+tax+net P&L, generate postmortem with MAE/MFE from vnstock (T+0/T+1 excluded). State under `state/vn_theses/` + `state/vn_journal/`. | 27 |
| **`vn-canslim-screener`** | CANSLIM (Mark Minervini) screener adapted for VN. 5 pillars: C/A (EPS YoY), N (52-week high within 5%), S (volume liquidity), L (RS rating / sector leadership), I (foreign net flow proxy). Status-flag filter for Kiểm soát/Hạn chế. Grade A/B/C. | 15 |
| **`vn-earnings-analyzer`** | Score recent earnings reactions on five factors: gap size, volume, 20-day pre-earnings trend, MA50/200 position, EPS surprise. VN-specific calibrations (gap thresholds tuned to ±7% band). Grade A→F. | 15 |
| **`vn-pead-screener`** | Post-earnings drift screener. Mode A standalone universe scan; Mode B pipeline from vn-earnings-analyzer. Computes proper VN ceiling/floor bands; warns when stop ≤ floor. R-multiple targets. | 15 |
| **`vn-margin-rules-monitor`** | Track per-broker margin-eligible lists, tier rates, Q-rated flags. Subcommands: `record` (ingest CSV), `check` (lookup w/ optional `--warn-q-rated`), `report` (full distribution), `history` (changes over time). State: `state/vn_margin/<broker>_margin_list.csv`. | 13 |
| **`vn-etf-screener`** | Score VN ETFs (VFMVN30, FUEVFVND, E1VFVN30, etc.) on tracking error, NAV premium/discount, expense ratio, 20-day volume. Grade A→F. Useful for Core portfolio passive backbone. | 12 |
| **`vn-daily-brief`** | Composite morning-routine orchestrator. Wraps `vn-sector-analyst`, `vn-foreign-room-tracker`, `vn-portfolio-manager` into one command; writes consolidated `reports/vn_daily_brief_<date>.md`. `--strict` flag for hard-fail on sub-skill error. | 8 |
| **Total VN tests** | | **~400** |

**Runnable example:** [`examples/vn-daily-swing/`](examples/vn-daily-swing/) — 7-step end-to-end demo of the deterministic VN skills (`bash examples/vn-daily-swing/run_demo.sh`).

See [`README.vi.md`](README.vi.md) for the Vietnamese quickstart with the end-to-end daily workflow.

### International (US — advanced track)
...
- **Market Breadth Analyzer** (`market-breadth-analyzer`)
  - Quantifies market breadth health using Xone.VN's public CSV data with a data-driven 6-component scoring system (0-100).
