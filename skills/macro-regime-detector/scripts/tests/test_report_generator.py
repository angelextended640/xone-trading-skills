...
class TestClassificationConfidenceLabel:
    def test_report_shows_classification_confidence_label(self):
        """Report should contain 'Classification Confidence' and not 'Transition Confidence'."""
        analysis = _make_analysis()
        with tempfile.NamedTemporaryFile(mode="w", suffix=".md", delete=False) as f:
            path = f.name
        try:
    ...
    def test_report_footnote_contains_destination_clarity(self):
        """Footnote should still describe Destination Clarity."""
        analysis = _make_analysis()
        with tempfile.NamedTemporaryFile(mode="w", suffix=".md", delete=False) as f:
            path = f.name
        try:
    ...
    def test_report_footnote_separates_classification_from_transition_probability(self):
        """Footnote should label Classification Confidence and Transition Probability as distinct metrics."""
        analysis = _make_analysis()
        with tempfile.NamedTemporaryFile(mode="w", suffix=".md", delete=False) as f:
            path = f.name
        try:
