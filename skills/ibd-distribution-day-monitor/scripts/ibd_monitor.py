...
def _build_explanation(symbol: str, d5: int, d15: int, d25: int, risk: str, today_dd: bool) -> str:
    parts = []
    parts.append(
        f"{symbol} today is {'a Distribution Day' if today_dd else 'not a Distribution Day'}."
    )
    parts.append(
        f"Active Distribution Days within 5/15/25 elapsed sessions: {d5}/{d15}/{d25}."
    )
    parts.append(f"Risk classification: {risk}.")
    return " ".join(parts)
...
def _build_cluster_state(d5: int, d15: int, d25: int) -> dict:
    return {
        ...
        "cluster_description": (f"within 5/15/25 elapsed sessions: {d5}/{d15}/{d25}"),
    }
