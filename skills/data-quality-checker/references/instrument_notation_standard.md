...
| Asset | Ticker (ETF) | Ticker (Futures) | Full Name | Notes |
|-------|-------------|-------------------|----------------|-------|
| Gold | GLD | GC | Gold | GLD = SPDR Gold Shares ETF; GC = COMEX Gold Futures |
| Silver | SLV | SI | Silver | SLV = iShares Silver Trust; SI = COMEX Silver Futures |
| S&P 500 | SPY | ES | S&P 500 | SPY = SPDR S&P 500 ETF; SPX = index; ES = E-mini Futures |
| S&P 500 Index | -- | SPX | S&P 500 Index | Cash index, not tradable directly |
| Volatility | -- | VIX | CBOE VIX | VIX = index; VXX/UVXY = ETPs |
| US Treasuries | TLT | ZB | 20+ Year Treasury | TLT = iShares 20+ Year; ZB = 30Y futures |
| Crude Oil | USO | CL | WTI Crude Oil | USO = United States Oil Fund; CL = NYMEX WTI Futures |
...
|----------|-------------|-------|
| USD/JPY | USDJPY | JPY/USD (reversed) |
| EUR/USD | EURUSD | USD/EUR (reversed) |
| GBP/USD | GBPUSD | USD/GBP (reversed) |
...
| Commodity | Futures Ticker | ETF Ticker |
|-----------|---------------|------------|
| Gold | GC | GLD, IAU |
| Silver | SI | SLV |
| Crude Oil (WTI) | CL | USO |
| Natural Gas | NG | UNG |
| Copper | HG | COPX |
...
4. **Localised documents** may use local-language names, but should
   include the ticker on first mention.
