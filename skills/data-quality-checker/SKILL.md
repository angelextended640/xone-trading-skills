...
2. **Section-aware allocation checking**: Only percentages within allocation
   sections (identified by headings like "Allocation" or table columns like
   "Weight" / "Target ratio") are checked. Random percentages in body text
   (probability, RSI, YoY growth) are ignored.

3. **Robust normalisation**: Full-width punctuation characters that occasionally
   appear in non-Latin reports (％, 〜, en-dash) are normalized before processing.
