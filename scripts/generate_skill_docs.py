...
GITHUB_REPO_URL = "https://github.com/xonevn-ai/xone-trading-skills"
...
def _generate_buttons(skill_name: str, skill_packages_dir: Path | None, lang: str = "en") -> str:
    ...
    Args:
        ...
        lang: kept for backwards compatibility; only "en" is supported.
...
def generate_page(
    ...
    # Build sections
    overview = _extract_section(sections, ["overview", title.lower(), "tổng quan"])
    if not overview:
        # Fallback: use the first paragraph of the body
        overview = skill_data["body"].split("\n\n")[0] if skill_data["body"] else description

    prerequisites = _extract_section(sections, ["prerequisites", "pre-requisites", "điều kiện tiên quyết"])
    workflow = _extract_section(sections, ["workflow", "running the script", "how to run", "hướng dẫn thực thi"])
    when_to_use = _extract_section(sections, ["when to use", "when to use this skill", "khi nào dùng"])
    ...
    parent_name = "Skill Guides" if lang == "en" else "Hướng dẫn Kỹ năng"
    grand_parent = "English" if lang == "en" else "Tiếng Việt"
    toc_label = "Table of Contents" if lang == "en" else "Mục lục"
    overview_lbl = "1. Overview" if lang == "en" else "1. Tổng quan"
    when_lbl = "2. When to Use" if lang == "en" else "2. Khi nào sử dụng"
    pre_lbl = "Prerequisites" if lang == "en" else "Điều kiện tiên quyết"
    quick_lbl = "Quick Start" if lang == "en" else "Bắt đầu nhanh"
    work_lbl = "Workflow" if lang == "en" else "Quy trình thực thi"
    res_lbl = "Resources" if lang == "en" else "Tài liệu tham khảo"
...
parent: {parent_name}
nav_order: {nav_order}
permalink: /{lang}/skills/{skill_name}/
---
...
def generate_en_page(*args, **kwargs):
    """Backward-compat alias for generate_page(..., lang='en').

    Pre-2026-05 the EN-specific 6-section generator was called `generate_en_page`.
    It was unified into `generate_page(lang=...)` during VI doc-site bring-up.
    Tests written against the old name still import this symbol.
    """
    kwargs.setdefault("lang", "en")
    return generate_page(*args, **kwargs)
...
def _replace_table_rows(index_path: Path, rows: list[str]) -> None:
    ...
    The table region runs from the line immediately after the `|---` separator
    up to (but not including) the next blank line or markdown heading. Lines
    that look like table rows are discarded; orphan non-table lines inside
    the region (typically multi-line description leftovers) are also discarded
    so the table cannot accumulate corruption across regenerations.
    """
...
_SLUG_RE = re.compile(r"/(?:en|vi)/skills/([\w-]+)/")
...
def _extract_catalog_slugs(text: str) -> set[str]:
    ...
    # From links like [Name](/en/skills/slug/) or [Name](/vi/skills/slug/)
...
def main(argv: list[str] | None = None) -> int:
    ...
    vi_dir = args.docs_dir / "vi" / "skills"
    ...
    for d in skill_dirs:
        ...
        vi_path = vi_dir / f"{name}.md"

        if en_path.exists() and vi_path.exists() and not args.overwrite:
            skipped += 1
            continue
        ...
        if args.mode == "full":
            # Generate 10-section skeleton pages (EN only; full mode is hand-curated)
            ...
            # Generate 6-section auto pages for both EN and VI
            en_content = generate_page(
                name,
                skill_data,
                api_info,
                cli_example,
        ...
        if not en_path.exists() or args.overwrite:
            en_path.write_text(en_content, encoding="utf-8")
            generated_en += 1
            print(f"  Generated: {name} (EN, mode={args.mode})")

        # VI generation (auto mode only — full mode is EN-only)
        if args.mode == "auto" and (not vi_path.exists() or args.overwrite):
            vi_content = generate_page(
                name,
                skill_data,
                api_info,
                cli_example,
            ...
            generated_vi += 1
            print(f"  Generated: {name} (VI, mode={args.mode})")

    print(f"\nDone: {generated_en} EN + {generated_vi} VI generated, {skipped} skipped")
