...
A comprehensive catalog of all 50 Xone Trading Skills organized by category. Badge indicators show API requirements at a glance.
{: .fs-6 .fw-300 }
...
## 0. Vietnam Market (Primary)

This is the primary skill family as of v0.2 (2026-05). All `vn-*` skills target HOSE / HNX / UPCOM, use VND, format reports in Asia/Ho_Chi_Minh, enforce lot-100 / tick-size / ±7-±10-±15% price band rules, and rely on the open-source [`vnstock`](https://github.com/thinh-vu/vnstock) data layer (no paid API keys required).

| Skill | Description | API Requirements |
|-------|-------------|-----------------|
| **[Vn Market Mechanics]({{ '/en/skills/vn-market-mechanics/' | relative_url }})** | Canonical reference — ±7/10/15% price bands, T+2.5 settlement, lot size 100, broker fees, 0.1% sale tax, foreign room. Every other `vn-*` skill cites this | <span class="badge badge-free">No API</span> |
| **[Vn Position Sizer]({{ '/en/skills/vn-position-sizer/' | relative_url }})** | Risk-based long-trade sizing with VN rules: lot-100 floor, ceiling/floor warning, fee+tax accounting. Fixed Fractional / ATR / Kelly | <span class="badge badge-free">No API</span> |
| **[Vn Data Fetcher]({{ '/en/skills/vn-data-fetcher/' | relative_url }})** | CLI wrapper around `vnstock`. Subcommands: `ohlcv`, `info`, `dividends`, `fundamentals`, `foreign-flow` snapshot. CSV cache + `--fixture` for offline tests | <span class="badge badge-free">No API</span> |
| **[Vn Portfolio Manager]({{ '/en/skills/vn-portfolio-manager/' | relative_url }})** | VND portfolio with after-fee/tax P&L, sector breakdown, 10%/30% concentration warnings. Pipes prices from vn-data-fetcher | <span class="badge badge-free">No API</span> |
| **[Vn Foreign Room Tracker]({{ '/en/skills/vn-foreign-room-tracker/' | relative_url }})** | Tracks foreign ownership room (VN-only concept) across a watchlist. Alerts: full / released / spike. Append-only history | <span class="badge badge-free">No API</span> |
| **[Vn Sector Analyst]({{ '/en/skills/vn-sector-analyst/' | relative_url }})** | VN-Index sector rotation across 11 sectors × ~80 most-traded stocks. Return 5D/20D/60D, RS, regime hint | <span class="badge badge-free">No API</span> |
| **[Vn VCP Screener]({{ '/en/skills/vn-vcp-screener/' | relative_url }})** | Minervini VCP adapted for HOSE ±7% bands. Successive contractions with declining depth + volume dry-up. Grade A/B/C | <span class="badge badge-free">No API</span> |
| **[Vn Pullback Screener]({{ '/en/skills/vn-pullback-screener/' | relative_url }})** | Lower-risk swing entry — pullback to MA20/MA50 with RSI 35-50 and volume dry-up | <span class="badge badge-free">No API</span> |
| **[Vn CANSLIM Screener]({{ '/en/skills/vn-canslim-screener/' | relative_url }})** | CANSLIM adapted for VN. 5 pillars: C/A (EPS), N (52w high), S (volume), L (RS/sector lead), I (foreign net-buy). Status-flag filter for Kiểm soát/Hạn chế. Grade A/B/C | <span class="badge badge-free">No API</span> |
| **[Vn Dividend Screener]({{ '/en/skills/vn-dividend-screener/' | relative_url }})** | Sustainable dividend stocks: yield + payout sustainability + 3y growth + ROE/D-E quality. Yield-trap auto-reject | <span class="badge badge-free">No API</span> |
| **[Vn ETF Screener]({{ '/en/skills/vn-etf-screener/' | relative_url }})** | Score VN ETFs (VFMVN30, FUEVFVND, E1VFVN30, …) on tracking error, NAV premium/discount, expense ratio, 20D volume. Grade A→F | <span class="badge badge-free">No API</span> |
| **[Vn Earnings Analyzer]({{ '/en/skills/vn-earnings-analyzer/' | relative_url }})** | Score recent earnings reactions on 5 factors: gap, volume, 20D pre-trend, MA50/200 position, EPS surprise. VN-calibrated. Grade A→F | <span class="badge badge-free">No API</span> |
| **[Vn PEAD Screener]({{ '/en/skills/vn-pead-screener/' | relative_url }})** | Post-earnings drift. Mode A standalone scan, Mode B pipeline from vn-earnings-analyzer. Computes VN ceiling/floor, warns when stop ≤ floor | <span class="badge badge-free">No API</span> |
| **[Vn Margin Rules Monitor]({{ '/en/skills/vn-margin-rules-monitor/' | relative_url }})** | Track per-broker margin-eligible lists, tier rates, Q-rated flags. Record/check/report/history subcommands | <span class="badge badge-free">No API</span> |
| **[Vn Breakout Trade Planner]({{ '/en/skills/vn-breakout-trade-planner/' | relative_url }})** | End-to-end long planner: pivot/stop/R-targets, T+2.5 sellable-day calendar, optional sector + foreign-room context | <span class="badge badge-free">No API</span> |
| **[Vn Tax Fee Calculator]({{ '/en/skills/vn-tax-fee-calculator/' | relative_url }})** | Itemised broker fees (10 CTCKs) + 0.1% sale tax + 5% dividend tax + custody + advance-cash. `compare` benchmarks net P&L across brokers | <span class="badge badge-free">No API</span> |
| **[Vn30 Derivatives Planner]({{ '/en/skills/vn30-derivatives-planner/' | relative_url }})** | VN30 Index Futures — `roll`, `hedge`, `plan`, `cost`. T+0 settlement, no sale tax. **The only legal short-exposure tool on VN equities** | <span class="badge badge-free">No API</span> |
| **[Vn News Analyst]({{ '/en/skills/vn-news-analyst/' | relative_url }})** | WebSearch-based VN news brief. Tier 1 sources (CafeF, Vietstock, NDH). Maps to affected tickers | <span class="badge badge-free">No API</span> |
| **[Vn Economic Calendar]({{ '/en/skills/vn-economic-calendar/' | relative_url }})** | SBV / GSO release schedule, FOMC dates that move USDVND, ex-rights, AGM season, Tết / quarter-end patterns | <span class="badge badge-free">No API</span> |
| **[Vn Trader Memory]({{ '/en/skills/vn-trader-memory/' | relative_url }})** | Plan → Trade → Record → Review thesis lifecycle. VND P&L with fees+tax, lot-100, vnstock-backed MAE/MFE (T+0/T+1 excluded) | <span class="badge badge-free">No API</span> |
| **[Vn Daily Brief]({{ '/en/skills/vn-daily-brief/' | relative_url }})** | Composite morning routine — wraps sector-analyst, foreign-room-tracker, portfolio-manager. Writes `reports/vn_daily_brief_<date>.md` | <span class="badge badge-free">No API</span> |
...
| Vn Economic Calendar | -- | -- | -- |
| Vn Dividend Screener | -- | -- | -- |
| Vn30 Derivatives Planner | -- | -- | -- |
| Vn Trader Memory | -- | -- | -- |
| Vn CANSLIM Screener | -- | -- | -- |
| Vn Earnings Analyzer | -- | -- | -- |
| Vn Margin Rules Monitor | -- | -- | -- |
| Vn PEAD Screener | -- | -- | -- |
| Vn Daily Brief | -- | -- | -- |
| Vn ETF Screener | -- | -- | -- |
