# Xone Trading Skills Project Vision

Version: 0.2
Last updated: 2026-05-13

Xone Trading Skills is a Claude Skills-based trading process OS for time-constrained individual investors.
...
## 0. Primary Market Focus — Vietnam (added 2026-05-13)

As of v0.2, the **primary audience of this project is Vietnamese retail traders and investors** working on HOSE / HNX / UPCOM. The project is being expanded with a family of `vn-*` skills built on the open-source `vnstock` data layer, with Vietnamese as the primary output language and explicit handling of VN market mechanics — ±7%/10%/15% daily price bands, lot size 100, T+2.5 settlement, foreign-room limits, broker fees, and 0.1% sale tax.

International skills (US/JP, FMP, FINVIZ, Alpaca) are **retained for advanced users** who trade multiple markets, but they are no longer the project's primary recommendation path. New users should expect:

- VN-first quickstart (`README.vi.md`)
- VN skills prefixed `vn-*` with Vietnamese-language SKILL.md and reports
- Shared knowledge base in `skills/vn-market-mechanics/` cited by all `vn-*` skills
- Existing US/JP skills remain in place; they may be re-cast as `us-*` or `jp-*` in a future major version

The phased migration plan (current state):

| Phase | Scope | Status |
| --- | --- | --- |
| 1 | Foundation: VN market mechanics reference + `vn-position-sizer` + repositioning docs | **In progress (2026-05)** |
| 2 | Core VN skills: `vn-data-fetcher`, `vn-portfolio-manager`, `vn-foreign-room-tracker`, `vn-sector-analyst` | Planned |
| 3 | Advanced VN skills: `vn-vcp-screener`, `vn-canslim-screener`, `vn-breakout-trade-planner` adapted for ±7% limit dynamics | Planned |
| 4 | VN-only concepts: `vn-tax-fee-calculator`, `vn30-derivatives-planner`, `vn-margin-rules-monitor` | Planned |
...
Xone Trading Skills is not a signal engine. It aims to be a **decision-process OS** that helps individual traders use AI to build a more resilient decision process while consistently managing judgment, risk, records, and improvement.

## 5. Purpose

Xone Trading Skills is a repository of Claude Skills focused on **Vietnamese stocks (HOSE/HNX/UPCOM) as the primary market**, with additional skills for US stocks, ETFs, dividend investing, swing trading, macro analysis, and strategy research, including options and event-driven strategies where appropriate. The international skills are retained for advanced users running multi-market workflows; the primary recommendation path for new users is the Vietnam track.
...
The primary audience is **time-constrained individual Vietnamese investors and swing traders** trading on HOSE / HNX / UPCOM. International (US/JP) traders remain a supported advanced audience.
...
- **Now**: Finish the project vision documents (`PROJECT_VISION.md`)
